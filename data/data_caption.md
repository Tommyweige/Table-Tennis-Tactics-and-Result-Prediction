...
本資料集以「小分（Rally）」為單位，完整記錄了桌球比賽中每一次擊球的狀態、技術動作與比賽脈絡資訊。

## 1. 檔案說明

| 檔案名稱 | 說明 |
| :--- | :--- |
| **train.csv** | 訓練資料集，包含完整標註（含預測目標）。 |
| **test.csv** | 測試資料集，不包含預測目標。 |
...
每一筆資料代表一場比賽中某一小分內的 **單次揮拍行為**，依時間順序排列。
> **注意：** `strikeId` 為本次競賽之**預測目標**。

| 欄位名稱 (Features) | 說明 | Definition |
| :--- | :--- | :--- |
...
| **serverGetPoint** | 發球者是否得分（1=是, 0=否）  (目標值)| Whether the server won the point |
...
| **pointId** | 球的落點位置 (目標值) | Landing position of the ball on the table |
| **actionId** | 擊球方式 (目標值) | Stroke or action type |
| **positionId** | 球員站位區域 | Player’s court position |

---

## 3. 分類定義表 (Category Mappings)
